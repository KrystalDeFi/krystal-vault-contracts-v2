...
contract SharedV3Strategy is ISharedStrategy {
  ...
  function _executeInstructions(bytes calldata data) internal returns (PositionChange[] memory changes) {
    ...
    if (instructions.whatToDo == IV3Utils.WhatToDo.COMPOUND_FEES) {
      ...
      emit CompoundFees(nfpm, tokenId, liquidity, added0, added1);
    ...
    if (instructions.whatToDo == IV3Utils.WhatToDo.CHANGE_RANGE) {
      ...
      emit ChangeRange(nfpm, tokenId, newTokenId, newLiquidity, added0, added1);
    ...
    if (instructions.whatToDo == IV3Utils.WhatToDo.WITHDRAW_AND_COLLECT_AND_SWAP) {
      ...
      emit WithdrawAndCollectAndSwap(nfpm, tokenId, instructions.targetToken, targetAmount);
  ...
  function _increasePosition(
    ...
    (liquidity, added0, added1) = INFPM(nfpm).increaseLiquidity(
  ...
  ///      getPositionAmounts. Without this, SharedVaultPreviewLib.verifyPositionExit's staticcall to
  ///      getPositionAmounts would receive amtsOk=false and block the untracking even for a legitimately
  ///      exited position on a non-standard NFPM that keeps ownerOf() working after the position is burned.
