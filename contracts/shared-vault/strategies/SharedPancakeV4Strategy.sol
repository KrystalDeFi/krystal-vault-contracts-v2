...
contract SharedPancakeV4Strategy is ISharedStrategy, IFeeTaker {
  ...
  ///      single-sided / out-of-range adds pass without spurious reverts.
  ...
  {
    (amount0, amount1) = SharedPancakeV4ValuationLib.getPositionAmounts(posm, tokenId);
  }
  ...
  function getPositionPrincipalAmounts(address posm, uint256 tokenId)
    ...
    returns (uint256 amount0, uint256 amount1)
  {
    (amount0, amount1) = SharedPancakeV4ValuationLib.getPositionPrincipalAmounts(posm, tokenId);
  }
  ...
  {
    (total0, total1, principal0, principal1) = SharedPancakeV4ValuationLib.getPositionAmountsSplit(posm, tokenId);
  }
