...
///      amount, so the `collected - fee` accounting downstream can never underflow. V4/Pancake call this
///      same helper directly, so the fee policy is unified across every shared strategy, eliminating the previous
