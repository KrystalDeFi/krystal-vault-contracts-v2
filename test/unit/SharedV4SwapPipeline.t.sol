...
contract SharedV4SwapPipelineTest is Test {
  ...
  /// @dev The input side is booked at the MEASURED delta (`total0 -= amountInDelta`), NOT the nominal
  ///      signed `amountIn`. Every other test approves and pulls the same amount, so amountInDelta ==
  ///      amountIn there and the two are indistinguishable. Here the router pulls LESS than the approved
  ///      signed amountIn (the digest still binds the full signedAmountIn and the partial swapData), so a
  ///      regression that booked the nominal amountIn would yield total0 == runtimeAmount - signedAmountIn
  ///      (== 0) and strand the unpulled input untracked. The correct contract books the measured pull.
  function test_execute_booksMeasuredDelta_notNominalAmountIn_whenRouterPullsPartial() public {
    ...
    uint256 amountOut = 5 ether;
    bytes memory rawSwapData =
      abi.encodeCall(SharedV4SwapPipelineRouter.swapPartial, (address(token0), address(token1), pull, amountOut));
  ...
  /// @dev TWO distinct non-pool inputs seed the ledger simultaneously (seedCount == 2): the second seed
  ///      slot, the array shrink to count 2, and a per-seed independent net-to-zero check are all
  ///      exercised for the first time — single-seed tests cannot reach them. Each seed is converted by
  ///      its own hop into a different pool token and must net to exactly zero.
  function test_executeWithInputs_twoDistinctNonPoolInputs_eachSeedConsumedToZero() public {
    SharedV4SwapPipelineTestToken tokenX = new SharedV4SwapPipelineTestToken("SourceX", "SRX");
    SharedV4SwapPipelineTestToken tokenY = new SharedV4SwapPipelineTestToken("SourceY", "SRY");
    harness.setVaultToken(address(tokenX), true);
    ...
    bytes memory hop0Data =
      abi.encodeCall(SharedV4SwapPipelineRouter.swapAll, (address(tokenX), address(token0), uint256(6 ether)));
    bytes memory hop1Data =
      abi.encodeCall(SharedV4SwapPipelineRouter.swapAll, (address(tokenY), address(token1), uint256(4 ether)));

    ISharedV4Utils.InputTokenParams[] memory inputs = new ISharedV4Utils.InputTokenParams[](2);
    inputs[0] = ISharedV4Utils.InputTokenParams({ token: Currency.wrap(address(tokenX)), amount: 6 ether });
    inputs[1] = ISharedV4Utils.InputTokenParams({ token: Currency.wrap(address(tokenY)), amount: 4 ether });
  ...
  /// @dev A zero-amountIn no-op hop whose tokenIn is a SEEDED (intermediate) token, not a pool token:
  ///      it must resolve the intermediate, pass the (0 <= tracked) budget check, then short-circuit
  ///      WITHOUT calling the router or decrementing the seed ledger. A second real hop consumes the
  ///      full seed so the ledger still nets to exactly zero. The signed amount (the whole tracked
  ///      seed) would have been resolved+verified+executed by the removed zero-sentinel — proving the
  ///      skip precedes signature/router work even on the intermediate branch.
  function test_executeWithInputs_zeroAmountInHopOnSeededInput_skipsWithoutTouchingLedger() public {
    SharedV4SwapPipelineTestToken tokenX = new SharedV4SwapPipelineTestToken("Source", "SRC");
    harness.setVaultToken(address(tokenX), true);
    tokenX.mint(address(harness), 10 ether);
    token0.mint(address(router), 6 ether);
  ...
  /// @dev The input gas fee applies to POOL-token inputs too, and the POST-fee amount is what folds
  ///      into the totals (no swap involved). Twin of the non-pool skim test above — that one proves
  ///      the fee skims from a seeded input; this proves a pool-token input's fold value is reduced by
  ///      the same skim. A regression folding the pre-fee amount (or skipping the skim for pool tokens)
  ///      would pass every other test.
  function test_executeWithInputs_skimsGasFeeFromPoolTokenInputThenFolds() public {
    address gasFeeRecipient = makeAddr("gasFeeRecipient");
    configManager.setFeeRecipient(gasFeeRecipient);
    ...
    ISharedV4Utils.InputTokenParams[] memory inputs = new ISharedV4Utils.InputTokenParams[](1);
    inputs[0] = ISharedV4Utils.InputTokenParams({ token: Currency.wrap(address(token0)), amount: 8 ether });
    ...
    (uint256 total0, uint256 total1) = harness.executeWithInputs(
      ...
      inputs,
      uint64(uint256(0x10000000000000000) / 4), // 25%
      new ISharedV4Utils.SwapParams[](0)
    );
  ...
  /// @dev Boundary of the dest exemption: a later hop consumes the ENTIRE dest output, so the dest
  ///      entry nets to zero. It must take the allowed-dest exemption branch (destOut = 0, no revert),
  ///      NOT the strict `else require(... == 0)` arm — distinguishing "dest fully consumed" from a
  ///      stranded non-dest intermediate.
  function test_executeToDest_destFullyConsumed_reportsZeroDestOut() public {
    SharedV4SwapPipelineTestToken tokenX = new SharedV4SwapPipelineTestToken("Dest", "DST");
    harness.setVaultToken(address(tokenX), true);
    token0.mint(address(harness), 10 ether);
    tokenX.mint(address(router), 5 ether);
