...
import { TestCommon, USER, WETH, USDC, SUSHI_NFPM as NFPM } from "../TestCommon.t.sol";
...
/// @notice Integration tests for SushiSwap V3 LP operations via SharedV3Strategy.
///         SushiSwap V3 is a Uniswap V3 fork: same positions() layout (uint24 fee at slot 4),
///         same IUniswapV3Factory.getPool(address,address,uint24) selector. No dedicated strategy needed.
contract SharedVaultSushiV3IntegrationTest is TestCommon {
  address constant V3_UTILS = 0xFb61514860896FCC667E8565eACC1993Fafd97Af;

  // SushiSwap V3 WETH/USDC 0.05% pool on Base
  ...
  // =========================================================
  // SWAP_AND_MINT: create a SushiSwap V3 LP position
  // =========================================================

  function test_sushi_swapAndMint_createsPosition() public {
    vm.startPrank(vaultOwner);
    ...
    assertEq(vault.getPositionCount(), 1, "should have 1 tracked Sushi LP position");
    uint256 tokenId = IERC721Enumerable(NFPM).tokenOfOwnerByIndex(address(vault), 0);
    assertGt(tokenId, 0, "tokenId must be non-zero");
    console.log("Sushi SWAP_AND_MINT: tokenId =", tokenId);
  ...
  function test_sushi_swapAndIncrease_addsLiquidity() public {
    vm.startPrank(vaultOwner);
    ...
    console.log("Sushi SWAP_AND_INCREASE: liquidity", liquidityBefore, "->", liquidityAfter);
  ...
  function test_sushi_withdraw_exitsProportionally() public {
    vm.startPrank(vaultOwner);
    ...
    console.log("Sushi withdraw: WETH =", withdrawn[0], "USDC =", withdrawn[1]);
  ...
  // shares — same block / same price, against the REAL SushiSwap V3 NFPM — must NEVER return more of
  // either token than was deposited (no value creation / no dilution of existing holders), and must
  // return all but AMM/rounding dust (so the no-profit bound is tight). Back-port of
  // Integration.SharedVault.V4.t.sol:672 onto the real Sushi V3 NFPM. Balanced-deposit path (no swap):
  // deposit amounts are proportional to the vault's current total balances (WETH/USDC ~3000:1, not 1:1),
  // so neither side is over-pulled. slippageBps=50 (0.5%) matches the V4 reference; dust = t*In/200.
  // =========================================================

  function test_sushi_depositWithdraw_roundTrip_neverProfits_onRealNfpm() public {
    // Establish a real Sushi V3 LP position first so the round-trip exercises the real NFPM
    ...
    address depositor = makeAddr("sushiRoundTripDepositor");
    ...
    // No profit: an instant round-trip cannot return more of either token than went in.
    ...
    console.log("Sushi round-trip: WETH in/out =", t0In, got[0]);
    console.log("Sushi round-trip: USDC in/out =", t1In, got[1]);
  }

  // =========================================================
  // Second depositor proportional shares with active Sushi LP
  // =========================================================

  function test_sushi_secondDepositor_proportional() public {
    vm.startPrank(vaultOwner);
    ISharedVault.Action[] memory mintActions = new ISharedVault.Action[](1);
    mintActions[0] = ISharedVault.Action(
      address(proxy),
    ...
    console.log("Sushi second depositor shares =", shares);
  }

  // =========================================================
  // previewWithdraw reflects Sushi LP value
  // =========================================================

  function test_sushi_previewWithdraw_includesLpValue() public {
    vm.startPrank(vaultOwner);
    ...
    assertGt(preview[0], 0, "WETH preview should reflect Sushi LP value");
    assertGt(preview[1], 0, "USDC preview should reflect Sushi LP value");
    console.log("Sushi previewWithdraw: WETH =", preview[0], "USDC =", preview[1]);
  ...
  function test_sushi_beaconUpgrade_newImplUsedWithoutRewhitelisting() public {
    vm.startPrank(vaultOwner);
    ...
    console.log("Sushi beacon upgrade: new impl =", address(newImpl));
  ...
  function _swapAndMintData(uint256 amount0, uint256 amount1) internal view returns (bytes memory) {
    ...
    IV3Utils.SwapAndMintParams memory params = IV3Utils.SwapAndMintParams({
      protocol: 0, // SushiSwap V3 is Uniswap V3-compatible
      nfpm: NFPM,
      token0: WETH,
      token1: USDC,
      fee: FEE_TIER,
