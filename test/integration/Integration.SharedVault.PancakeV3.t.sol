...
contract SharedVaultPancakeV3IntegrationTest is TestCommon {
  ...
  function setUp() public {
    ...
    vault = SharedVault(payable(vaultFactory.createVault("SharedVault-PancakeV3", vaultTokens, initialAmounts, 0)));
  ...
  // shares — same block / same price, against the REAL PancakeSwap V3 NFPM — must NEVER return more of
  ...
  // Integration.SharedVault.V4.t.sol:672 onto the real Pancake V3 NFPM. Balanced-deposit path (no swap):
  ...
  function test_pancake_depositWithdraw_roundTrip_neverProfits_onRealNfpm() public {
    // Establish a real Pancake V3 LP position first so the round-trip exercises the real NFPM
    ...
    address depositor = makeAddr("pancakeRoundTripDepositor");
    ...
    console.log("Pancake round-trip: WETH in/out =", t0In, got[0]);
    console.log("Pancake round-trip: USDC in/out =", t1In, got[1]);
  }
