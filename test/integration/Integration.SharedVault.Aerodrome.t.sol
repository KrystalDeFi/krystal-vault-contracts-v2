...
contract SharedVaultAerodromeIntegrationTest is TestCommon {
  ...
  // shares — same block / same price, against the REAL Aerodrome NFPM — must NEVER return more of either
  ...
  // value). Back-port of Integration.SharedVault.V4.t.sol:672 onto the real Aerodrome CL NFPM.
  // Uses the balanced-deposit path (no swap): deposit amounts are proportional to the vault's current
  // total balances (the WETH/USDC pool is ~3000:1, not 1:1), so neither side is materially over-pulled
  // and the per-side dust bound stays tight. slippageBps=50 (0.5%) matches the V4 reference, so the dust
  // fraction is t*In/200 (0.5%).
  // =========================================================

  function test_aerodrome_depositWithdraw_roundTrip_neverProfits_onRealNfpm() public {
    // Establish a real Aerodrome LP position first, so the deposit/withdraw round-trips through the
    // real NFPM (increaseLiquidity -> exitProportional), not just idle balances.
    vm.startPrank(vaultOwner);
    ISharedVault.Action[] memory mintActions = new ISharedVault.Action[](1);
    mintActions[0] = ISharedVault.Action(
      address(aeroProxy),
    ...
    address depositor = makeAddr("aeroRoundTripDepositor");
    ...
    console.log("Aerodrome round-trip: WETH in/out =", t0In, got[0]);
    console.log("Aerodrome round-trip: USDC in/out =", t1In, got[1]);
  }
