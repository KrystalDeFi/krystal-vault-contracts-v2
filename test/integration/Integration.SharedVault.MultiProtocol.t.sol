...
contract SharedVaultMultiProtocolIntegrationTest is TestCommon {
  ...
  // shares — same block / same price, against BOTH real NFPMs (UniswapV3 + PancakeV3) simultaneously —
  ...
  // Back-port of Integration.SharedVault.V4.t.sol:672 across a multi-position vault: the deposit's
  // proportional top-up fans out into both tracked positions, so this conservation check covers the
  // multi-NFPM increaseLiquidity -> exitProportional path. Balanced-deposit (no swap): deposit amounts
  // are proportional to the vault's current total balances (WETH/USDC ~3000:1, not 1:1). slippageBps=50
  // (0.5%) matches the V4 reference; dust = t*In/200.
  // =========================================================

  function test_multiProtocol_depositWithdraw_roundTrip_neverProfits_onRealNfpms() public {
    // Establish one position on each real NFPM first so the round-trip exercises both
    // (increaseLiquidity on each -> exitProportional on each), not just idle balances.
    vm.startPrank(vaultOwner);
    ISharedVault.Action[] memory mintActions = new ISharedVault.Action[](2);
    mintActions[0] = ISharedVault.Action(
      address(v3Proxy),
      _uniV3MintData(0.4 ether, 1200e6),
      ISharedCommon.CallType.DELEGATECALL
    );
    mintActions[1] = ISharedVault.Action(
      address(pancakeProxy),
      _pancakeMintData(0.4 ether, 1200e6),
      ISharedCommon.CallType.DELEGATECALL
    );
    vault.execute(mintActions);
    assertEq(vault.getPositionCount(), 2, "two positions before round-trip");
    vm.stopPrank();

    address depositor = makeAddr("multiRoundTripDepositor");
    ...
    console.log("Multi-protocol round-trip: WETH in/out =", t0In, got[0]);
    console.log("Multi-protocol round-trip: USDC in/out =", t1In, got[1]);
  }
