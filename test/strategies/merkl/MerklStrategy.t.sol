...
contract MerklStrategyTest is TestCommon {
    ...
    function setUp() public {
        ...
        rewardToken.transfer(address(distributor), 100000 * 10**18);
        
        // Deploy mock swap router and fund it with principal tokens
        swapRouter = new MockSwapRouter();
        principalToken.transfer(address(swapRouter), 100000 * 10**18);
        
        // Deploy config manager
        configManager = new ConfigManager(owner);
    ...
    function testRevertWhenSwapRouterNotWhitelisted() public {
        ...
        principalToken.transfer(address(nonWhitelistedRouter), 100000 * 10**18);
        ...
        AssetLib.Asset[] memory inputAssets = new AssetLib.Asset[](0);
        vm.expectRevert(InvalidSwapRouter.selector);
