...
usage="yarn <deploy,deploy-private,deploy-shared,test> [-h] [-c <eth,bsc,polygon,arbitrum,optimism,base,berachain,ronin,hyperevm,robinhood,arc>] [-n <mainnet,testnet>] -- to run test on specific chain and network

where:
    -h  show this help text
    -c  which chain to run, supported <eth,bsc,polygon,arbitrum,optimism,base,berachain,ronin,hyperevm,robinhood,arc>
...
while getopts ":hc:n:f:x:" option; do
  case $option in
    h) 
      ...
      ;;
    c) 
      if [[ ! "$OPTARG" =~ ^(eth|bsc|polygon|arbitrum|optimism|base|berachain|ronin|hyperevm|robinhood|arc)$ ]]; then
