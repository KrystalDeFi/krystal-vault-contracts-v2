...
```solidity
constructor(address _oldStrategy, address _newStrategy) public
```
...
```solidity
function convert(struct AssetLib.Asset[] assets, struct ICommon.VaultConfig config, struct ICommon.FeeConfig feeConfig, bytes data) external payable returns (struct AssetLib.Asset[] result)
```
