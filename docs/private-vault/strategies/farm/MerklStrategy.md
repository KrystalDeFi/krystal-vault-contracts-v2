...
```solidity
function claimMerkleReward(address token, uint256 amount, bytes32[] proofs, uint64 rewardFeeX64, uint64 gasFeeX64, bool vaultOwnerAsRecipient) external payable
```
